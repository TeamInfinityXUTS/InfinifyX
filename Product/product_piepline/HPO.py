...
def hpo_step(graph_cache='graph_cache.pt', graph_cache_model_id=None, graph_cache_task_id='e42c8c37d17a402384607e9308e5a53e', graph_cache_artifact_name='graph_cache', n_trials=3, max_epochs_per_trial=2, val_split=0.15, test_split=0.15):
    import torch
    import numpy as np
    ...
    logger.report_text('GNN HYPERPARAMETER OPTIMIZATION (HPO)')
    logger.report_text('=' * 100)
    ...
    def resolve_task_artifact(task_id, artifact_name):
        ...
        return validate_file(source_task.artifacts[selected_name].get_local_copy(), f'ClearML artifact {selected_name}')
    ...
    class GNNClassifier(nn.Module):
        ...
        def __init__(self, hidden_dim=128, dropout=0.3):
            ...
            self.classifier = nn.Sequential(nn.Linear(hidden_dim, 64), nn.ReLU(), nn.Dropout(dropout), nn.Linear(64, 3))
    ...
    local_search_space = [{'hidden_dim': 64, 'dropout': 0.1, 'lr': 0.001, 'batch_size': 32}, {'hidden_dim': 64, 'dropout': 0.15, 'lr': 0.0005, 'batch_size': 16}, {'hidden_dim': 128, 'dropout': 0.2, 'lr': 0.0005, 'batch_size': 16}, {'hidden_dim': 128, 'dropout': 0.3, 'lr': 0.0003, 'batch_size': 32}, {'hidden_dim': 128, 'dropout': 0.25, 'lr': 0.001, 'batch_size': 8}, {'hidden_dim': 256, 'dropout': 0.3, 'lr': 0.0003, 'batch_size': 16}, {'hidden_dim': 256, 'dropout': 0.4, 'lr': 0.0001, 'batch_size': 32}, {'hidden_dim': 256, 'dropout': 0.35, 'lr': 5e-05, 'batch_size': 64}]
    ...
    for trial_idx, cfg in enumerate(search_space):
        ...
        for epoch in range(max_epochs_per_trial):
            ...
            _, _, val_f1, _ = precision_recall_fscore_support(val_labels_true, val_preds, average='macro', zero_division=0)
        ...
        trial_results.append({'trial_id': trial_idx + 1, 'config': cfg, 'val_f1': best_val_f1_trial, 'val_loss': best_val_loss_trial, 'epochs': epoch + 1})
    ...
    test_precision, test_recall, test_f1, _ = precision_recall_fscore_support(test_labels_true, test_preds, average='macro', zero_division=0)
    test_class_precisions, test_class_recalls, test_class_f1s, _ = precision_recall_fscore_support(test_labels_true, test_preds, labels=[0, 1, 2], average=None, zero_division=0)
    test_report = classification_report(test_labels_true, test_preds, labels=[0, 1, 2], target_names=['Low', 'Medium', 'High'], digits=3, zero_division=0)
    ...
    ax.set(xticks=np.arange(3), yticks=np.arange(3), xticklabels=['Low', 'Medium', 'High'], yticklabels=['Low', 'Medium', 'High'], ylabel='True', xlabel='Predicted', title='Confusion Matrix (Test)')
    thresh = cm.max() / 2.0
    for i in range(3):
        for j in range(3):
    ...
    for i, result in enumerate(sorted_results):
        ...
        logger.report_text(f"{rank:<8} {cfg['hidden_dim']:<10} {cfg['dropout']:<10.2f} {cfg['lr']:<12.0e} {cfg['batch_size']:<6} {result['val_f1']:<10.4f} {result['val_loss']:<12.4f} {result['epochs']:<8}")
    logger.report_text('-' * 100)
    ...
    logger.report_text(f"{'=' * 100}")
    return {'best_config': best_cfg, 'val_f1': float(best_val_f1), 'test_f1': float(test_f1), 'test_accuracy': float(test_accuracy)}
...
if __name__ == '__main__':
    ...
    kwargs = {'graph_cache': 'graph_cache.pt', 'graph_cache_model_id': None, 'graph_cache_task_id': 'e42c8c37d17a402384607e9308e5a53e', 'graph_cache_artifact_name': 'graph_cache', 'n_trials': 3, 'max_epochs_per_trial': 2, 'val_split': 0.15, 'test_split': 0.15}
    task.connect(kwargs, name='kwargs')
    function_input_artifacts = {}
    params = task.get_parameters(cast=True) or dict()
    argspec = inspect.getfullargspec(hpo_step)
    ...
    if '0' in kwargs:  # *args arguments are present
        ...
        results = hpo_step(*pos_args, **kwargs)
    else:
        results = hpo_step(**kwargs)
    result_names = ('return_object',)
    if result_names:
